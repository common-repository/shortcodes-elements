...
define("SE_PLUGIN_DIR_PATH", plugin_dir_path(__FILE__));

include(SE_PLUGIN_DIR_PATH . 'shortcodes_func.php');
include(SE_PLUGIN_DIR_PATH . 'shortcodes_code.php');
...
{
    //dashboard menu page
    add_menu_page('Weblizar Shortcode Elements', 'Weblizar Shortcode Elements', 'manage_options', 'esweblizar_system', 'esweblizar_page_content', 'dashicons-layout', '59');
...
function esweblizar_page_content()
{
    ...
    wp_enqueue_style('font-awesome', E_S_WEBLIZAR . 'font-awesome/css/all.min.css');
}
...
function esweblizar_script_assets()
{
    wp_enqueue_script('jquery');
    // Bootstrap
    wp_enqueue_style('bootstrap', E_S_WEBLIZAR . 'css/bootstrap.min.css', array(), true);
    wp_enqueue_script('popper', E_S_WEBLIZAR . 'js/popper.min.js', array('jquery'), true);
    wp_enqueue_script('bootstrap', E_S_WEBLIZAR . 'js/bootstrap.min.js', array(), true);
    //css files
    wp_enqueue_style('shortcode-css', E_S_WEBLIZAR . 'css/shortcode-css.css', array(), true);
    wp_enqueue_style('font-awesome.min-css', E_S_WEBLIZAR . 'font-awesome/css/all.min.css', array(), true);
    //js files
...
{
    $links[] = '<a href="' .
        admin_url('admin.php?page=esweblizar_setting') .
        '">' . __('Settings') . '</a>';
    return $links;
}
