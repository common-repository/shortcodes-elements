...
function esweblizar_shotcode_gallery_shortcode_element($atts, $content = null)
{
	...
	if ($gallery_type == 'one') {
		...
		<img class="img-responsive" src="' . $url . '" alt="' . $url . '" />
			...
			if ($count % 3 == 0) {
				$out .= '	 <div class="col-md-12"></div>';
			}
			$count++;
		}
		$out .= '</div></div>';
	} elseif ($gallery_type == 'three') {
		$out = '<div class="weblizar_gallery_shortcode grid ' . $gallery_class . '" ><div class="row">';
		foreach ($am as $url) {
...
function esweblizar_carousel_shortcode_element($atts, $content = null)
{
	...
	$out .= '			</div>
				     ...
				     <a class="carousel-control-prev" href="#myCarousel-' . $rand . '" data-slide="prev"><span
				                class="carousel-control-prev-icon"></span></a>
				    <a class="carousel-control-next" href="#myCarousel-' . $rand . '"data-slide="next"><span
...
function esweblizar_vimeo_shortcode_element($atts, $content = null)
{
	...
	$out = '<iframe src="' . $v_url . '" width="' . $vimeo_width . '" height="' . $vimeo_height . '" frameborder="0" allow="' . $v_autoplay . '; fullscreen" allowfullscreen></iframe>';
	return $out;
}
add_shortcode('weblizar_vimeo', 'esweblizar_vimeo_shortcode_element');
...
function esweblizar_gmap_shortcode_element($atts, $content = null)
{
	...
	$rand = rand(1, 100);
	$map_location = $map_id;
	$map_axes = str_replace(', ', ' ', $map_location);
	...
	$out .= 'var styles = {
      ...
      function toggleBounce() {
		...
		<script async defer
		src="https://maps.googleapis.com/maps/api/js?key=' . isset($google_api['google_api']) . '&callback=initMap">
	    </script>';
	return $out;
}
...
function esweblizar_frame_function_element($atts, $content = null)
{
	...
	$out = '<style>';
	if ($align == 'right') {
		$out .= '.weblizar_frame_shotcode{ float: right; }';
	} elseif ($align == 'left') {
		$out .= '.weblizar_frame_shotcode{ float: left; }';
	} elseif ($align == 'center') {
		$out .= '.weblizar_frame_shotcode img.frame_image{ margin: auto; };
		';
	}
	$out .= '</style>
...
function esweblizar_tabs_shortcode_element($atts, $content = null)
{
	...
	if ($tabs_style == '1') {
		...
		for ($i = 1; $i <= $fields; $i++) {
			if ($i == 1) {
				$out .= '    <li role="presentation" class="nav-item active"><a class="nav-link" href="#tab-' . $rand . '-' . $i . '" aria-controls="tab-' . $rand . '-' . $i . '" role="tab" data-toggle="tab">' . $title[$i] . '</a></li>';
			} else {
				$out .= '   <li role="presentation" class="nav-item"><a class="nav-link" href="#tab-' . $rand . '-' . $i . '" aria-controls="tab-' . $rand . '-' . $i . '" role="tab" data-toggle="tab">' . $title[$i] . '</a></li>';
			}
		}
		$out .= '</ul>
          <!-- Tab panes -->
...
add_shortcode('highlighter', 'esweblizar_heighlighter_shortcode_element');

/* Blog */
function esweblizar_blog_shortcode_element($atts, $content = null)
{
	ob_start();
	$atts = shortcode_atts(
		array(
		...
		<div class=" gallary-3">
			...
			<div class=" gallary animate-grid">
				...
				<div class="gallary-thumbs row lightgallery  wow fadeInUp" data-wow-delay="0.6s" id="main">
					...
					?>
							<div class="col-lg-<?php echo esc_attr($col); ?> col-md-<?php echo esc_attr($col); ?> col-sm-4 col-xs-12 allwork space-1 <?php foreach ($categories as $category) {
																																								echo 'cat-' . $category->term_id . ' ';
																																							} ?> ">
								<div class="isotop-gallery">
									<?php if (has_post_thumbnail()) { ?>
										<div class="socialmedia-inner">
											<?php $default = array('class' => "img-responsive");
											the_post_thumbnail('medium', $default);  ?>
										</div>
									<?php } ?>
									<div class="overlap-11">
										<a href="<?php the_permalink(); ?>">
											<h2 class="blog_template4_title"><?php the_title(); ?></h2>
										</a>
										<p>
											...
											<span><?php if (('d M  y') == get_option('date_format')) : ?>
												...
												<?php endif; ?></span>
											<span><?php $pid = get_the_ID();
													$args = array('post_id' => $pid, 'count' => true);
													$comments = get_comments($args);
													echo esc_html($comments) ?> Comments</span>
										</p>
...
function esweblizar_expand_shortcode_element($atts, $content = null)
{
	...
	<style>
		...
		.read-more-state~.read-more-trigger:before {
			content: '<?php if ($show_more_title != '') echo esc_html($show_more_title);
						else echo 'Show more'; ?>';
		}

		.read-more-state:checked~.read-more-trigger:before {
			content: '<?php if ($show_less_title != '') echo esc_html($show_less_title);
						else echo 'Show less'; ?>';
		}
